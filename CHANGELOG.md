...
- Batch deletion now hard-deletes with cascade instead of archiving; batch settings includes a danger-zone "Delete batch" action gated behind a name-confirmation dialog
- Tilt polling status accurately reflects running state across Next.js module contexts by persisting `tilt.lastPollTime` to the DB and deriving `running` from recency rather than an in-memory flag
- Adding the first Tilt hydrometer now auto-enables polling without requiring a separate settings toggle
- Protocol tab correctly displays "Gravity stable for 24h" instead of "3 readings" when `stableDurationHours` is set; seed templates and default criteria builder updated to include `stableDurationHours: 24`
